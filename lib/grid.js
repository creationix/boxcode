...
define("grid", function () {
  ...
  function Leaf(grid, parent) {
    ...
    function onDragOver(evt) {
      ...
      console.log("over leaf", leaf);
      evt.preventDefault();
      leaf.grid.drag.targetLeaf = leaf;
      leaf.grid.drag.targetTab = null;
    }
  ...
  Leaf.prototype.add = function (tab, insertIndex) {
    // Build a label for this tab
    var label = {};
    domBuilder([".tab$el", {draggable: true, ondragover: ondragover, ondragstart: onDragStart, ondragend: ondragend},
    ...
    function ondragend(evt) {
      ...
      // Swap places wih target tab if there is one
      if (drag.targetTab) {
        drag.targetLeaf = drag.targetTab.leaf;
        // Remove target tab
        var targetIndex = drag.targetLeaf.tabs.indexOf(drag.targetTab);
        var index = drag.leaf.tabs.indexOf(drag.tab);
        drag.targetLeaf.add(drag.tab, targetIndex);
        drag.leaf.add(drag.targetTab, index);
        drag.leaf.select(drag.targetTab);
        drag.targetLeaf.select(drag.tab);
      }
      // Otherwise Move to target leaf
      else if (drag.targetLeaf) {
    ...
    function ondragover(evt) {
      if (leaf.grid.drag.tab === tab) return;
      console.log("over tab", tab);
      evt.preventDefault();
      evt.stopPropagation();
      leaf.grid.drag.targetTab = tab;
    }
    ...
    if (insertIndex !== undefined) {
      var oldLabel = this.labels[insertIndex];
      this.labels[insertIndex] = label;
      this.tabs[insertIndex] = tab;
      this.tabviewEl.replaceChild(label.el, oldLabel.el);
    }
    else {
