...
define("grid", function () {
  ...
  Leaf.prototype.add = function (tab) {
    ...
    var label = {};
    domBuilder([".tab$el",
      ["span$span", {title: tab.title, draggable: true},
        ["i$icon", {onclick: onMenu, class: tab.icon}],
    ...
    function onMenu(evt) {
      evt.preventDefault();
      evt.stopPropagation();
      if (!this.menu) this.menu = new Menu(tab);
      var left = label.el.offsetLeft;
      var top = label.el.offsetTop + label.el.offsetHeight - 2;
      var width = label.el.offsetWidth - 2;
      leaf.select(tab);
      this.menu.show(left, top, width);
    }
  ...
  Leaf.prototype.onkeydown = function (evt) {
    if (this.selected) {
      // Check for custom shortcuts in the menu
      var menu = this.selected.menu;
      if (menu) {
        loop: for (var i = 0, l = menu.length; i < l; i++) {
          ...
          for (var key in shortcut) {
            if (evt[key] != shortcut[key]) continue loop;
          }
          evt.preventDefault();
          evt.stopPropagation();
  ...
  Menu.prototype.show = function (x, y, width) {
    ...
    var el = domBuilder(["ul.menu", {css: {left: x + "px", top: y + "px", minWidth: width + "px"}}, this.items.map(function (item) {
